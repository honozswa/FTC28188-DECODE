...
public class BlueV6OpMode extends OpMode {
    ...
    public void init() {
        ...
        if (PoseConstant.hasAutoPose) {
            ...
            follower.setStartingPose(PoseConstant.BlueFarAutoStartPose);
        }
    ...
    public void Aimbot(LLResult result) {
        if (gamepad1.left_bumper && gamepad1.left_trigger > 0.5) {
            ...
            if (result.isValid()) {
                ...
                double error = Util.angleWrap(robotPose.getHeading() - targetHeading + ShooterConstant.odoOffset + ShooterConstant.blueOdoOffset  + odoOffset);
