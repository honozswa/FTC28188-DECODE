...
@TeleOp(name = "V4-OdoAim", group = "Tester")
public class V4Odo extends OpMode {
...
//            TurretAim(result);
    ...
    public void odoAim() {
        ...
        if (turretOn) {
            ...
            double targetAngle = drive.getAngleToGoal(robotPose, GOAL);
            double robotHeading = robotPose.getHeading();
            double robotTarget = targetAngle - robotHeading;
            double turretAngle = Math.toRadians(turret.getCurrentAngle());
            double error = Util.angleWrap(robotTarget - turretAngle);
            if (Math.abs(error) < TurretConstant.angleTolerance) {
                turretPower = 0;
                lastErrorOdo = 0;
            } else {
